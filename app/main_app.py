# -*- coding: utf-8 -*-
"""
Created on Fri Feb 23 01:50:43 2024
...
loaded_model = pickle.load(open("C:/Users/ankit/.vscode/vscode/ML/heart/model/trained_model.pkl",'rb'))
...
def main():
    ...
    if st.button('Diagonasis Test Result'):
        diagnosis = heart_prediction([float(age), float(sex), float(cp), float(restbps), float(chol), float(fbs), float(restecg), float(thalach), float(exang), float(oldpeak), float(slope), float(ca), float(thal)])
